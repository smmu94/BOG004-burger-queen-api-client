...
console.log(resetKitchenOrder)
    if (resetKitchenOrder.find(p => p.id === id)) {
      resetKitchenOrder.splice(
        resetKitchenOrder.findIndex(p => p.id === id),1);
    }
  ...
  return (
    <div >
      ...
      <div className="order-container">{order.filter((o)=> o.status === "pending").map(ord =>{
        return(
          <Kitchen key={'order-' + ord.id} id={ord.id} client={ord.client} product={ord.products}  dataEntry={ord.dataEntry} resetKitchen={resetKitchen} />
        )
      })}</div>
    </div>
  );
